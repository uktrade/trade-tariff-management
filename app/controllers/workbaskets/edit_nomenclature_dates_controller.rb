module Workbaskets
  class EditNomenclatureDatesController < Workbaskets::BaseController
    skip_around_action :configure_time_machine, only: [:submitted_for_cross_check]

    expose(:sub_klass) { "EditNomenclatureDates" }
    expose(:settings_type) { :edit_nomenclature_dates }
    ...
    expose(:submitted_url) do
      submitted_for_cross_check_edit_nomenclature_date_url(workbasket.id)
    end

    expose(:form) do
      WorkbasketForms::EditNomenclatureDatesForm.new(original_nomenclature)
    end
    ...
    def edit
      @edit_nomenclature_dates_form = WorkbasketForms::EditNomenclatureDatesForm.new(original_nomenclature, workbasket.settings)
    end

    def update
      params[:settings] = params[:workbasket_forms_edit_nomenclature_dates_form]
      if saver.save!

        workbasket.submit_for_cross_check!(current_admin: current_user)
        redirect_to submitted_for_cross_check_edit_nomenclature_date_url(workbasket.id)
      else
        @edit_nomenclature_dates_form = WorkbasketForms::EditNomenclatureDatesForm.new(original_nomenclature, workbasket.settings)
        render :edit
      end
