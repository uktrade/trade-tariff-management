...
class FootnoteValidator < TradeTariffBackend::Validator
  validation :FO1, 'The referenced footnote type must exist.', on: %i[create update] do
    validates :presence, of: :footnote_type_id
  end

  validation :FO2, 'The combination footnote type and code must be unique.', on: %i[create update] do
    validates :uniqueness, of: %i[footnote_type_id footnote_id]
  end

  validation :FO3, 'The start date must be less than or equal to the end date.', on: %i[create update] do
    validates :validity_dates
  end

  validation :FO4, 'At least one description record is mandatory.
    ...
    The start date must be less than or equal to the end date of the footnote.', on: %i[create update] do |record|
  ...
  validation :FO5, 'When a footnote is used in a measure the validity period of the footnote must span the validity period of the measure.', on: %i[create update] do |record|
  ...
  validation :FO8, 'When a footnote is used in an Additional code the validity period of the footnote must span the validity period of the association with the Additional code.', on: %i[create update] do |record|
  ...
  validation :FO10, 'When a footnote is used in a Meursing Table heading the validity period of the footnote must span the validity period of the association with the Meursing heading.', on: %i[create update] do
  ...
  validation :FO17, 'The validity period of the footnote type must span the validity period of the footnote.', on: %i[create update] do
